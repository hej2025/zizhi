...
durationSec: 5|10|15
outputDir: string                         # 输出目录（默认 .claude/ai_videos/<projectId>/assets/）
retryLimit: number
```
...
## 通用资源关联
- 默认复用 `.claude/resources/douyin_video_common/poll_master_generic.sh` 执行持续轮询与下载。
- 需要 10 分钟心跳时，默认复用 `.claude/resources/douyin_video_common/queue_report_10min_generic.sh`。
...
- 权限不足时进入“非生成降级模式”，可执行 `.claude/scripts/fallback_not_vip.sh` 生成交付模板。
