...
## 输入/输出约定
- 输入：`projectId`、`traceId`、`finalVideoPath`、`subtitlePath`、`audioFiles`、`preprodPackage`、`postprodResult`、`coverImagePath`、`reviewRound`、`modelPolicy`。
- 输出：`status`、`errorCode`、`verdict`、`overallScore`、`dimensions[]`（8 维度）、`redoActions[]`、`userConfirmationPrompt`、`awaitingUserConfirmation`、`nextStep`、`ruleReview`、`semanticReviewTriggered`、`reviewModelUsed`。

## 审核策略（两级模式）
- 第一级：规则审核（默认必跑）
   - 检查项：字幕与配音偏差、字幕安全区、首帧有效性、音频编码与响度、色彩连续性、BGM 情绪匹配。
   - 规则审核通过且无关键告警时，可直接进入用户确认，不触发语义审核。
- 第二级：语义审核（条件触发）
   - 触发条件：规则审核出现 `warn/fail`，或 `redoActions` 归因不明确，或 `modelPolicy.qualityTier=premium`。
   - 常规使用 `creative_standard`；`creative_premium` 仅用于高价值项目最终复判。
- 升级上限：单项目语义高级复判最多 1 次，超限后仅允许局部修复与人工确认。
...
2. 先执行规则审核并输出 `ruleReview`（不依赖高成本模型）。
3. 满足触发条件时再调用 `video_quality_reviewer` 执行语义审核（8 个维度）。未触发时跳过语义审核并沿用规则审核结果。
4. 根据审核结果决定流转方向：
...
5. 返回结构化审核报告，包含每个维度的评分、具体问题、修复建议。
6. **生成用户确认提示**（当 verdict 为 APPROVED / APPROVED_WITH_WARNINGS / APPROVED_FORCE 时）：
