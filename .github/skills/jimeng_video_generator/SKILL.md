...
8. 队列轮询：必须采用 `while true` 持续轮询并定期输出心跳（建议 10 分钟一次），同时维护 ETA 状态文件供人工查看与恢复执行。
...
## 通用资源关联
- 默认复用 `.github/resources/douyin_video_common/poll_master_generic.sh` 执行持续轮询与下载。
- 需要 10 分钟心跳时，默认复用 `.github/resources/douyin_video_common/queue_report_10min_generic.sh`。
- 若资源目录脚本存在，不再在 `tmp/` 下临时重复生成同类脚本。
...
- 权限不足时进入“非生成降级模式”，可执行 `.github/scripts/fallback_not_vip.sh` 生成交付模板。
