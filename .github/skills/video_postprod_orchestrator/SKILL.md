...
## 成本策略与模型边界
- 后期阶段默认脚本优先，不引入高成本 LLM 参与字幕、配音、混音、调色、封面与首帧修复。
- 允许模型参与的场景仅限：最终人工交接摘要或失败复盘说明，且建议使用 `cheap_structured`。
- 若 `modelPolicy.qualityTier=premium`，也不得绕过后期规则检查与自检门。
...
- **输入映射**：
  ...
  outputDir         ← <projectDir>/audio/dubbing/
  ```
...
### Step 2: 声音设计与混音（D.1.2 — 依赖 Step 1 配音完成）
- **调用**：`sound_designer`
- **输入映射**：
  ```
...
- 自检报告必须输出结构化字段，作为 review_agent 规则审核输入，避免重复语义推理。
...
| 打回指令 | 路由目标 | 重跑范围 | 说明 |
|---------|---------|---------|------|
| `REDO_DUBBING` | Step 1 → Step 2 → Step 5 | 重新配音→重新混音→重新剪辑 | 配音质量问题 |
...
## 通用资源关联
- 默认优先复用 `.github/resources/douyin_video_common/auto_finalize_generic.sh` 进行自动收敛导出。
- 发布包结构默认对齐 `.github/resources/douyin_video_common/publish_package.template.json`。
...
## 注意事项
- **并行执行**：Step 1+2（D.1 声音设计）和 Step 3（D.2 调色）可并行执行；但 Step 1 是 Step 2 的前置依赖（配音先于混音）。Step 5（D.3 剪辑）必须等待 D.1 和 D.2 全部完成。
...
- **模型边界**：后期修复环节不得把规则问题外包给高成本模型解释；优先依据结构化信号执行固定修复动作。
...
- **降级策略**：
  - `sound_designer` BGM 生成失败 → 降级为仅配音 + 静态 BGM 背景（最基本可听版本）。
