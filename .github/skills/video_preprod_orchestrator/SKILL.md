---
name: video_preprod_orchestrator
...
modelPolicy: object                   # 主 Agent 下发的模型策略（可选）
preprodCacheKey: string               # 规范化缓存键（可选，建议由主Agent生成）
```

## 成本与模型路由策略
- 默认 `qualityTier=balanced`，若未显式传入则按平衡档执行。
- 模型职责建议：
  - `cheap_structured`：参数规范化、校验、摘要、结构重排。
  - `creative_standard`：脚本创作、关键分镜文本。
  - `creative_premium`：仅用于复杂叙事项目（60s/多角色/高创意品牌故事）。
- 当 `qualityTier=lean` 且满足「15s/30s + 单角色 + 信息型内容」时，允许跳过独立剧情结构化步骤（Step 2a）。
- 每次执行前优先尝试读取 `preprodCacheKey` 对应缓存；命中后先 `validate_only`，通过再放行到 Phase C。
...
> 轻量模式：若满足 `qualityTier=lean` + 简单信息型条件，Step 2a 可跳过，`storyPlan` 由脚本段落直接映射生成最小结构（beats/emotionCurve）。
...
- 推荐重跑优先级：`validate_only` > `storyboard` / `character` > `story` > `script`。
- 非必要禁止全量重跑：仅在脚本主旨变化、时长目标变化或质量门连续失败时回退到 `script`。

## Prompt 压缩约束（生成友好）
- storyboard 建议输出 `shotDelta`（镜头差异字段）而非每镜头完整长 Prompt。
- 公共字段（`globalVisualStyle`、`characterVisualAnchors`、`aspectRatio`、`style_reference`）在下游统一拼装，避免 token 重复。

## 通用资源关联
- 产出字段必须可直接供 `.github/resources/douyin_video_common/` 脚本链路消费（特别是时长预算与字幕断句建议）。
