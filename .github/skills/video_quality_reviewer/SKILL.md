...
- **封面质量**: 封面是否从有效帧抽取
  ...
  - FAIL: 封面为黑帧或占位图
- **打回指令**: `REDO_FIRST_FRAME` — 裁剪黑帧开头; `REDO_COVER` — 从 30%-50% 位置重新抽帧
...
3. **逐维度审核**: 按上述 6 个维度逐一审核，每个维度输出 `PASS` / `WARN` / `FAIL` 及具体数据。
4. **综合判定**:
...
- redoActions:                               # 仅 REJECTED 时非空
  - action: REDO_SUBTITLE|REDO_SUBTITLE_LAYOUT|REDO_FIRST_FRAME|REDO_COVER|REDO_GENERATION|REDO_BGM|REDO_AUDIO
    reason: <string>                         # 打回原因
