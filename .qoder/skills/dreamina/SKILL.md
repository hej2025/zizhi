...
## Concurrency & parallel submission

- **普通模型**（`seedance2.0fast`、`seedance2.0`）：同一账号同时只能有 **1 个任务** 在生成队列中，第二个提交会返回 `ret=1310 ExceedConcurrencyLimit`。必须串行：等一个完成再提交下一个。
- **VIP 模型**（`seedance2.0fast_vip`、`seedance2.0_vip`）：**并发上限预设 20 个任务**（实测无硬性限制，预设上限防止异常批量提交），可同时提交多个任务并行生成和轮询。每任务 55 credits（普通 25）。
- **推荐策略**：多镜头项目优先使用 VIP 模型并行提交所有镜头，然后并行轮询（每个镜头一个后台进程），大幅缩短总等待时间。
- **轮询并行模式**：对每个 `submit_id` 启动独立轮询进程（`poll_one <submit_id> <shot_name> &`），使用 `wait` 等待全部完成。
