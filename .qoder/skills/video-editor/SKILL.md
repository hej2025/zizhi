...
## 共享枚举引用
> 本 Skill 使用的枚举值定义于 `.qoder/skills/_shared/enums.md`，包括 TransitionType、ShotType、VfxType、Emotion。
...
outputDir: string                         # 输出目录（默认 .qoder/ai_videos/<projectId>/final/）
```
...
## 通用资源关联
- 当使用通用导出链路时，优先复用 `.qoder/resources/douyin_video_common/auto_finalize_generic.sh`。
