---
name: video-postprod-orchestrator
...
- **输入映射**：
  ...
  outputDir         ← .qoder/ai_videos/<projectId>/audio/dubbing/
  ```
...
- **输入映射**：
  ...
  outputDir         ← .qoder/ai_videos/<projectId>/audio/narration/
  ```
- **执行流程**：
...
- **输入映射**：
  ...
  outputDir         ← .qoder/ai_videos/<projectId>/final/
  ```
...
## 通用资源关联
- 默认优先复用 `.qoder/resources/douyin_video_common/auto_finalize_generic.sh` 进行自动收敛导出。
- 发布包结构默认对齐 `.qoder/resources/douyin_video_common/publish_package.template.json`。
